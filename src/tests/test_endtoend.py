...
def test_tcp():
    ...
    proc = subprocess.Popen(['./statsrelay', '--verbose', '--config=tests/statsrelay_tcp.conf', '--bind=127.0.0.1:8125'])
    time.sleep(0.1)
    proc.send_signal(1)
    time.sleep(0.1)
...
def test_udp():
    ...
    proc = subprocess.Popen(['./statsrelay', '--verbose', '--config=tests/statsrelay_udp.conf', '--bind=127.0.0.1:8125'])
    time.sleep(0.1)
    proc.send_signal(1)
    time.sleep(0.1)
...
def main():
    ret = test_udp() 
    if ret != 0:
        print 'FAIL: %s test_udp' % __file__
        return ret
    else:
        print 'PASS: %s test_udp' % __file__

    ret = test_tcp()
    if ret != 0:
        print 'FAIL: %s test_tcp' % __file__
        return ret
    else:
        print 'PASS: %s test_tcp' % __file__
